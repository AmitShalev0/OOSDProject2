...
namespace TravelExpertsMaintenance
{
    public partial class frmProductsSupplier : Form
    {
        ...
        //        using (TravelExpertsContext db = new TravelExpertsContext())
        //        {
        //            // remove from BookingDetails
        //            var bookingDetails = db.BookingDetails.Where(bd => bd.ProductSupplierId == selectedPS.ProductSupplierId);
        //            foreach (var bd in bookingDetails)
        //            {
        //                db.BookingDetails.Remove(bd);
        //            }

        //            // remove from ProductsSuppliers
        //            var productsSuppliers = db.ProductsSuppliers.Where(ps => ps.ProductSupplierId == selectedPS.ProductSupplierId);
        ...
        //            var suppliers = db.Suppliers.Where(s => s.SupplierId == selectedPS.SupplierId);
        //            foreach (var s in suppliers)
        //            {
        //                db.Suppliers.Remove(s);
        //            }

        //            // remove from SupplierContacts
        //            var supplierContacts = db.SupplierContacts.Where(sc => sc.SupplierId == selectedPS.SupplierId);
        //            foreach (var sc in supplierContacts)
        //            {
        //                db.SupplierContacts.Remove(sc);
        //            }


        //            // remove the ProductsSupplier record
        ...
        private void DeletePS()
        {
            ...
            if (result == DialogResult.Yes)
            {
                using (TravelExpertsContext db = new TravelExpertsContext())
                {
                    DeleteEntities(db.BookingDetails, bd => bd.ProductSupplierId == selectedPS.ProductSupplierId);
                    DeleteEntities(db.ProductsSuppliers, ps => ps.ProductSupplierId == selectedPS.ProductSupplierId);
                    DeleteEntities(db.Suppliers, s => s.SupplierId == selectedPS.SupplierId);
                    DeleteEntities(db.SupplierContacts, sc => sc.SupplierId == selectedPS.SupplierId);
